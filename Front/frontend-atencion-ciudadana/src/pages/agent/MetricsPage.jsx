    ...
    <header className="border-b border-slate-200 bg-white px-5 py-5 md:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            <p className="mb-1 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-blue-700">
              <BarChart3 className="h-3.5 w-3.5" /> Informes y rendimiento
            </p>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Métricas de atención</h1>
            <p className="mt-1 text-sm text-slate-500">Analizá tendencias, cumplimiento y demanda para tomar decisiones.</p>
          </div>
          <div className="relative flex flex-wrap gap-2"><button onClick={() => setFiltersOpen((open) => !open)} aria-expanded={filtersOpen} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"><SlidersHorizontal className="h-4 w-4" /> Filtros{activeFilterCount > 0 && <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#0F2C59] px-1 text-[11px] text-white">{activeFilterCount}</span>}<ChevronDown className={`h-4 w-4 transition-transform ${filtersOpen ? "rotate-180" : ""}`} /></button>
            ...
            {filtersOpen && 
              <div className="absolute right-0 top-12 z-30 w-[min(92vw,34rem)] rounded-xl border border-slate-200 bg-white p-4 shadow-xl">
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-bold text-slate-900">Filtrar métricas</p>
                    <p className="text-xs text-slate-500">Combiná los criterios para actualizar los indicadores.</p>
                  </div>
                  <button onClick={() => setFiltersOpen(false)} className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="Cerrar filtros">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="text-xs font-semibold text-slate-600">Período
                    <select value={days} onChange={(e)=>setDays(Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700">
                  ...
                  <label className="text-xs font-semibold text-slate-600">Prioridad
                    <select value={priority} onChange={(e)=>setPriority(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700">
                      <option value="">Todas las prioridades</option>{Object.entries({ LOW: "Baja", MEDIUM: "Media", HIGH: "Alta", CRITICAL: "Crítica" }).map(([id, name])=><option key={id} value={id}>{name}</option>)}
                    </select>
                  </label>
                  <label className="text-xs font-semibold text-slate-600">SLA
                    <select value={sla} onChange={(e)=>setSla(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700">
                      {SLA_FILTERS.map((item)=><option key={item.value} value={item.value}>{item.label}</option>)}
                    </select>
                  </label>
                  <label className="text-xs font-semibold text-slate-600">Área responsable
                    <select value={area} onChange={(e)=>setArea(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700">
                      <option value="">Todas las áreas</option>{areas.map((id)=><option key={id} value={id}>{FALLBACK_AREA_NAMES[id] || id}</option>)}
                    </select>
                  </label>
                </div>
                <div className="mt-4 flex justify-end">
                  <button onClick={resetFilters} className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800"><RotateCcw className="h-3.5 w-3.5" /> Restablecer filtros</button>
                </div>
              </div>}
          </div>
        </div>
      <div className="mt-4 flex items-center gap-2 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-800">
        ...
        <span>Esta vista es analítica e histórica. Para el trabajo del día y alertas en tiempo real, usá el <Link to="/agente/dashboard" className="font-bold underline">Dashboard operativo</Link>.</span>
      </div>
    </div>
    </header>
